...
> **English spoken status decision (user instruction, 2026-09-26):** The spoken status route (A5) also runs in English. The call greets in the page's language, and the caller's first answer is transcribed with Whisper's language detection; the call then continues in Bangla, or in English when at least two English words were heard (so a lone "হ্যাঁ" heard as "Happy" does not switch it). This keeps a Bangla speaker on a browser that defaults to English in Bangla. English turns use Whisper with `language: en` and English hints, the same fixed templates written in English (numbers as words), and are spoken by the caller's browser (Web Speech API); BanglaTTS still speaks only Bangla, and no new service, key, or model was added. Every other rule of the 2026-09-25 decision stands: fixed templates, no name, matter, lawyer, or office, the privacy question before the PIN, and ID plus PIN. The 16699 intake call stays Bangla with recorded prompts.
>
