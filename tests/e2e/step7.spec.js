...
test('Step 7 Nuching offline intake survives loss, syncs once, resolves a conflict, and has a cited document briefing', async ({ page, request }) => {
  ...
  await expect(page.locator('.plain-list li').filter({ hasText: /Draft/i }).first()).toBeVisible()

  await page.context().setOffline(true)
  await expect(page.getByRole('status').filter({ hasText: /No internet connection|Connection: offline/i })).toBeVisible()
  await page.getByLabel(/Original (?:words|statement)/).fill('Fictional Marma account continued after the network was lost.')
  await page.getByRole('button', { name: /Save application on this device|Queue encrypted application/i }).click()
  await expect(page.locator('.plain-list li').filter({ hasText: /Waiting to send|Queued/i })).toHaveCount(1)
  for (const number of [2, 3]) {
    await page.getByRole('button', { name: /(?:Fill with )?fictional Nuching example/i }).click()
    await page.getByLabel(/Applicant.*name/i).fill(`Fictional Nuching Offline ${number}`)
    await page.getByRole('button', { name: /Save application on this device|Queue encrypted application/i }).click()
    await expect(page.locator('.plain-list li').filter({ hasText: /Waiting to send|Queued/i })).toHaveCount(number)
  }
  await page.getByRole('button', { name: /Check saved drafts|Verify local integrity/i }).click()
  await expect(page.getByRole('status').filter({ hasText: /3 local encrypted drafts? verified/i })).toBeVisible()

  await page.context().setOffline(false)
  await expect(page.getByText(/No saved drafts yet|No local drafts/i)).toBeVisible({ timeout: 30000 })
  ...
  await page.getByRole('button', { name: /Correct an application/i }).click()
  await page.getByLabel('Application ID').fill(applicationId)
  await page.getByRole('button', { name: /Load application to correct|Open limited correction/i }).click()
  await expect(page.getByRole('heading', { name: new RegExp(`Correct.*${applicationId}`) })).toBeVisible()
  await page.context().setOffline(true)
  await page.getByLabel(/Original (?:words|statement)/).fill('Fictional corrected Marma words from a local UDC draft.')
  await page.getByRole('button', { name: /Save correction on this device|Queue encrypted correction/i }).click()
  ...
  await page.context().setOffline(false)
  await expect(page.getByRole('heading', { name: /Review the two versions|Human conflict review/i })).toBeVisible({ timeout: 30000 })
  await expect(page.getByText('Fictional corrected Marma words from a local UDC draft.')).toBeVisible()
  await page.getByLabel(/Why are you choosing this version\?|Reason for human choice/i).fill('I compared both fictional versions and retained the new local words as a revision.')
  await page.getByRole('button', { name: /Save device version as a new correction|Apply local as new revision/i }).click()
  await expect(page.getByRole('status').filter({ hasText: 'Human resolution recorded' })).toBeVisible()
  await expect(page.getByText(/No saved drafts yet|No local drafts/i)).toBeVisible()
...
test('Step 7 assisted intake remains labeled and keyboard reachable on a narrow screen', async ({ page }) => {
  ...
  await page.getByLabel(/Device passphrase|Local draft passphrase/i).fill('FictionalSecretPhrase!')
  await page.getByLabel(/Device passphrase|Local draft passphrase/i).focus()
  await page.keyboard.press('Tab')
  await expect(page.getByRole('button', { name: 'Start a new application' })).toBeFocused()
})
