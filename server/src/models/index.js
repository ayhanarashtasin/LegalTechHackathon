...
const mediationSchema = new Schema({
  ...
  caseId: { type: String, required: true },
  officeCode: { type: String, required: true },
  // Optional: appointed by the DLAO officer, who stays the case owner. With no mediator, the DLAO officer runs the steps.
  mediatorUserId: ref('User', false),
  appointedByUserId: ref('User', false),
  appointedAt: Date,
  // Who filled in each step form (DLAO officer or appointed mediator); the audit chain keeps the full history.
  filledBy: { type: Map, of: new Schema({ role: { type: String, enum: ['DLAO_OFFICER', 'MEDIATOR'] }, userId: ref('User'), at: Date }, { _id: false }) },
  ...
  safetyConsent: {
    safeForApplicant: Boolean,
    applicantAgreed: Boolean,
    applicantAvailable: Boolean,
    oppositePartyWilling: Boolean,
    status: { type: String, enum: ['CONSENT_CONFIRMED', 'NOT_SAFE', 'CONSENT_PENDING'] },
    reason: { type: String, maxlength: 500 },
    confirmedByUserId: ref('User', false),
    confirmedAt: Date,
  },
  sessionType: { type: String, enum: ['JOINT', 'SEPARATE_APPLICANT', 'SEPARATE_RESPONDENT'], default: 'JOINT' },
  language: { type: String, maxlength: 100, default: 'Bangla' },
  participants: [{ type: String, maxlength: 200 }],
  ...
  sessionDetails: {
    applicantComplaint: { type: String, maxlength: 2000 },
    applicantRequestedSolution: { type: String, maxlength: 2000 },
    applicantStatements: { type: String, maxlength: 2000 },
    oppositePartyResponse: { type: String, maxlength: 2000 },
    oppositePartyPosition: { type: String, maxlength: 2000 },
    oppositePartyProposedSolution: { type: String, maxlength: 2000 },
    issuesDiscussed: [{ type: String, maxlength: 500 }],
    proposedSolutions: [{ type: String, maxlength: 500 }],
    mediatorNotes: { type: String, maxlength: 3000 },
  },
  settlementTerms: [{
    issue: { type: String, required: true, maxlength: 200 },
    applicantAsks: { type: String, required: true, maxlength: 300 },
    oppositePartyOffers: { type: String, required: true, maxlength: 300 },
    agreedTerm: { type: String, maxlength: 500 },
    status: { type: String, enum: ['AGREED', 'DISAGREED', 'UNDER_NEGOTIATION'], default: 'UNDER_NEGOTIATION' },
  }],
  outcome: { type: String, enum: ['AGREEMENT_REACHED', 'NO_AGREEMENT', 'CONTINUED', 'NOT_SAFE'] },
  ...
  certifiedAt: Date,
  claoReturnReason: { type: String, maxlength: 1000 },
  claoReturnedAt: Date,
  followUp: {
    dueDate: Date,
    settlementFollowed: Boolean,
    paymentStatus: { type: String, enum: ['PAYMENT_MADE', 'PAYMENT_NOT_MADE', 'PAID', 'NOT_PAID', 'PARTIAL', 'NOT_APPLICABLE'] },
    agreementComplied: Boolean,
    furtherAssistanceRequired: { type: String, maxlength: 1000 },
    actionTaken: { type: String, enum: ['CLOSED', 'REOPENED_REFERRED', 'KEEP_MONITORING', 'CLOSE_CASE', 'REOPEN_REFERRAL'] },
    recordedByUserId: ref('User', false),
    recordedAt: Date,
  },
  sessions: [{
    ...
    recordedByUserId: ref('User', false),
    recordedByRole: { type: String, enum: ['DLAO_OFFICER', 'MEDIATOR'] },
    createdAt: { type: Date, default: Date.now },
  }],
  createdByUserId: ref('User'),
}, { timestamps: true })
...
const signatureRecordSchema = new Schema({
  ...
  authorizationMethod: { type: String, enum: ['PARTY_CODE', 'MEDIATOR_SESSION', 'DLAO_AS_MEDIATOR', 'CLAO_SESSION'] },
